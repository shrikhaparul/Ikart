...
def store_data(result_df, destination,arguments):
    """
    ...
    try:
        ...
        if destination["target_type"]=="Local Server":
            if destination['file_type'] == 'csv':
                store_data_to_csv(result_df, destination)
            elif destination['file_type'] == 'json':
                store_data_to_json(result_df, destination)
            elif destination['file_type'] == 'xml':
                store_data_to_xml(result_df, destination)
            elif destination['file_type'] == 'parquet':
                store_data_to_parquet(result_df, destination)
            elif destination['file_type'] == 'excel':
                store_data_to_excel(result_df, destination)
            else:
                task_logger.error(NOT_COMPATIBLE)
                raise SyntaxError(NOT_COMPATIBLE)
        elif destination['target_type'] == 'PostgreSQL':
            store_data_to_db_postgres(result_df,
                                    destination['connection_name'],
                                    destination['schema'],
                                    destination['table_name'],arguments)
        elif destination['target_type'] == 'MYSQL':
            store_data_to_db_mysql(result_df,
                                    destination['connection_name'],
                                    destination['table_name'],arguments)
        elif destination['target_type'] == 'MSSQL':
            store_data_to_db_mssql(result_df,
                                    destination['connection_name'],
                                    destination['table_name'],arguments)
        else:
            task_logger.error(NOT_COMPATIBLE)
...
def store_data_to_csv(df, destination):
    """
    ...
    """
    try:
        csv_location=destination['file_path']
        ...
        if destination["compression"]!="":
            compress_file(csv_location, destination["compression"])
...
def store_data_to_db_mysql(df,connection_name, table,arguments):
    """
...
def store_data_to_db_mssql(df,connection_name, table,arguments):
    """
    Store DataFrame to a SQL database.
...
def store_data_to_db_postgres(df,connection_name, schema, table,arguments):
    """
    Store DataFrame to a SQL database.
...
def store_data_to_json(df, destination):
    """
    ...
    """
    try:
        json_location=destination['file_path']
        ...
        if destination["compression"]!="":
            compress_file(json_location, destination["compression"])
...
def store_data_to_xml(df, destination):
    """
    ...
    """
    try:
        xml_location=destination['file_path']
        ...
        if destination["compression"]!="":
            compress_file(xml_location, destination["compression"])
...
def store_data_to_parquet(df, destination):
    """
    ...
    """
    try:
        parquet_location=destination['file_path']
        ...
        if destination["compression"]!="":
            compress_file(parquet_location, destination["compression"])
...
def store_data_to_excel(df, destination):
    """
    ...
    """
    try:
        excel_location=destination['file_path']
        ...
        if destination["compression"]!="":
            compress_file(excel_location, destination["compression"])
