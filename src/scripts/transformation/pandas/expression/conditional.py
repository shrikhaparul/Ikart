...
def perform_coalesce(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        ...
        if match:
            ...
            for col in columns[1:]:
                if col in temp_df_header:
                    df.loc[:, expression['output_col_name']] = df[expression['output_col_name']
                        ].fillna(temp_df[col]).combine_first(temp_df[col])
                elif col in df.columns:
                    df.loc[:, expression['output_col_name']] = df[expression['output_col_name']
                        ].fillna(df[col]).combine_first(df[col])
                else:
                    df.loc[:, expression['output_col_name']] = df[expression['output_col_name']
                    ].fillna(col)
        ...
        task_logger.exception("Error occured in perform_coalesce function with error : %s",e)
        audit_failure(arguments)
        raise e

def perform_case_expression(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in perform_case_expression function with error : %s",e)
        audit_failure(arguments)
        raise e

def apply_nullif_tempdf(arguments,df,expression,temp_df,temp_df_header,columns):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in apply_nullif_tempdf function with error : %s",e)
        audit_failure(arguments)
        raise e

def apply_nullif_df(arguments,df,expression,temp_df,temp_df_header,columns):
    """
    Apply a NULLIF-like operation to a DataFrame.
    ...
    try:
        ...
        task_logger.exception("Error occured in apply_nullif_df function with error : %s",e)
        audit_failure(arguments)
        raise e

def perform_nullif(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        ...
        if match:
            ...
            if columns[0] in temp_df_header:
                df=apply_nullif_tempdf(arguments,df,expression,temp_df,temp_df_header,columns)
            elif columns[0] in df.columns:
                df=apply_nullif_df(arguments,df,expression,temp_df,temp_df_header,columns)
            else :
        ...
        task_logger.exception("Error occured in perform_nullif function with error : %s",e)
        audit_failure(arguments)
        raise e

def conditional_exp(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        match expression['operator']:
            ...
            # Coalesce returns the first non-null value among the specified columns
                df=perform_coalesce(arguments,temp_df, temp_df_header, df, expression)
            case 'case expression':
            # Case expression applies conditions and assigns values accordingly
                df=perform_case_expression(arguments,temp_df, temp_df_header, df, expression)
            case 'nullif':
                df=perform_nullif(arguments,temp_df, temp_df_header, df, expression)
            case _:
                raise ValueError(f"Unsupported operation type: {expression['operator']}")
        return temp_df, df
    except Exception as e:
        task_logger.exception("Error occured in conditional_exp function with error : %s",e)
        audit_failure(arguments)
        raise e
