...
def extract_datepart(arguments,input_string, df,temp_df):
    """
    ...
    try:
        ...
        date_functions = {
            ...
            'timezone_hour': lambda x: x.dt.tz_convert(None).dt.tz_localize(None).dt. \
            tz_convert('UTC').dt.tz_convert(x.dt.tz).dt.hour,
            'timezone_minute': lambda x: x.dt.tz_convert(None).dt.tz_localize(None).dt.\
            tz_convert('UTC').dt.tz_convert(x.dt.tz).dt.minute
        }
        ...
        task_logger.exception("Error occured in extract_datepart function with error : %s",e)
...
def pyspark_to_pandas_format(arguments,pyspark_format):
    ...
    try:
        ...
        task_logger.exception("Error occured in pyspark_to_pandas_format function with error:%s",e)
        audit_failure(arguments)
        raise e
...
def frequency_alias(word):
    ...
    try:
        ...
        task_logger.exception("Error occured in frequency_alias function with error : %s",e)
        raise e

def extract_words(arguments,input_string):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in extract_words function with error : %s",e)
        audit_failure(arguments)
        raise e
...
def get_date(arguments,df,expression):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in get_date \
         function with error : %s",e)
        audit_failure(arguments)
        raise e

def calculate_months_between(arguments,d1, d2):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in calculate_months_between \
         function with error : %s",e)
        audit_failure(arguments)
        raise e

def apply_months_between(arguments,df,expression,temp_df):
    """
    ...
    try:
        ...
        if match:
            ...
            if (date1 in df.columns or date1 in temp_df.columns) and (
                ...
                df[expression['output_col_name']]=df.apply(lambda row:
                    calculate_months_between(arguments,row[date1],row[date2]),axis=1)
            elif date1 in df.columns or date1 in temp_df.columns    :
                df[expression['output_col_name']] = df[date1].apply(
                    lambda d1: calculate_months_between(arguments,d1,date2))
            elif date2 in df.columns or date2 in temp_df.columns:
                df[expression['output_col_name']] = df[date2].apply(
                lambda d2: calculate_months_between(arguments,date1,d2))
            else:
                df[expression['output_col_name']] = calculate_months_between(arguments,date1, date2)
        ...
        task_logger.exception("Error occured in apply_months_between function with error : %s",e)
        audit_failure(arguments)
        raise e


def apply_date_trunc(arguments,df,temp_df,expression):
    """
    ...
    try:
        ...
        if match:
            values = extract_words(arguments,expression['expression_value'])
        else:
            raise SyntaxError
        try:
        ...
        task_logger.exception("Error occured in apply_date_trunc function with error : %s",e)
        audit_failure(arguments)
        raise e

def apply_timestamp(arguments,df,temp_df,expression):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in apply_timestamp function with error : %s",e)
        audit_failure(arguments)
        raise e


def apply_datediff(arguments,df,match,expression):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in apply_datediff function with error : %s",e)
        audit_failure(arguments)
        raise e


def appy_extract(arguments,df,temp_df,expression):
    """
    ...
    try:
        ...
        df[expression['output_col_name']]=extract_datepart(arguments,extracted_text,df,temp_df)
        return df
    except Exception as e:
        task_logger.exception("Error occured in appy_extract function with error : %s",e)
        audit_failure(arguments)
        raise e


def gregorian(arguments,df,expression):
    ...
    try:
        ...
        task_logger.exception("Error occured in gregorian function with error : %s",e)
        audit_failure(arguments)
        raise e


def date_time_exp(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        ...
        match expression['operator']:
            case 'datediff':
                ...
                if match:
                    df=apply_datediff(arguments,df,match,expression)
                else:
                    task_logger.error("Error in syntax of datetime")
                    raise SyntaxError
            case 'getdate':
                df=get_date(arguments,df,expression)
            case 'months_between':
                df=apply_months_between(arguments,df,expression,temp_df)
            case 'date_trunc':
                df = apply_date_trunc(arguments,df,temp_df,expression)
            case 'to_timestamp':
                df=apply_timestamp(arguments,df,temp_df,expression)
            case 'extract':
                df = appy_extract(arguments,df,temp_df,expression)
            case 'to_julian':
                df = add_or_update_column(arguments,df ,temp_df ,expression['output_col_name'],
            expression['input_col_name'],'')
                original_string = expression['expression_value']
    ...
    # Format Julian Date string
            ...
            case "to_gregorian":
                df = add_or_update_column(arguments,df ,temp_df ,expression['output_col_name'],
            expression['input_col_name'],'')
                df=gregorian(arguments,df,expression)
            case _:
                raise ValueError("Unsupported operation type: %s",expression['operator'])
        return temp_df, df
    except Exception as e:
        task_logger.exception("Error occured in date_time_exp function with error : %s",e)
        audit_failure(arguments)
        raise e
