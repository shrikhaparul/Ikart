...
def get_clauses(arguments,syntax_parts):
    """
    ...
    try:
        ...
        task_logger.exception("Error occured in get_clauses function with error : %s",e)
        audit_failure(arguments)
        raise e

def add_row_number(arguments,temp_df, temp_df_header, df, syntax):
    """
    ...
    try:
        ...
        if len(syntax_parts) > 1:
            partition_clause,order_by_clause=get_clauses(arguments,syntax_parts)
        ...
        if partition_clause is not None and order_by_clause is not None:
            ...
            row_number=df.groupby(partition_cols)[order_by_clause].rank(
            method='first',ascending=False)
            return  row_number.astype(int)
        raise SyntaxError("Invalid syntax of row_number function")
    except Exception as e:
        task_logger.exception("Error occured in add_row_number function with error : %s",e)
        audit_failure(arguments)
        raise e

def window_exp(arguments,temp_df, temp_df_header, df, expression):
    """
    ...
    try:
        ...
        match operator_name:
            case 'row_number':
                ...
                temp_df, expression['output_col_name'],
                                        expression['input_col_name'],
                                        '')
                df[output_column]=add_row_number(arguments,temp_df,
                temp_df_header, df, expression['expression_value'])
    except Exception as e:
