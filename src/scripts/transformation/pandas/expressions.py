...
def extract_columns(transformations):
...
def find_operator_type(operation):
    ...
    try:
        ...
        raise NotImplementedError(f"Operation type {operation.get('operator_type','Unknown')} not implemented") from e

def store_columns_in_temp(df, temp_df, temp_df_header, input_col):
    """
...
def perform_rename_drop(df,input_columns,output_columns):
    """
...
def expression(df, operations):
    ...
    """
    try:
        input_columns,output_columns=extract_columns(operations)
        ...
        for operation in expression_list:
            if operation['operatorType']!="":
                operation_func = find_operator_type(operation)
                ...
                if operation['input_col_name']==operation['output_col_name']:
                    temp_df_header, temp_df = store_columns_in_temp(df,
                                                                    temp_df,
                                                                    temp_df_header,
                                                                    operation['input_col_name'])
                task_logger.info("%s has been apllied",operation['expression_value'])
                temp_df, df = operation_func(temp_df, temp_df_header, df, operation)
        task_logger.info(df.head())
        df=perform_rename_drop(df,input_columns,output_columns)
        return df
    except KeyError as e:
