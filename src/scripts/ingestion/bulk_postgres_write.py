""" script for writing data from postgresql table"""
import sys
import logging
import os
...
def insert_data(conn_details,dataframe,sessions,target):
    ...
    try:
        ...
        if target["audit_fields"] == "YES":
            ...
            dataframe.to_sql(target["object_name"].lower(), connection, schema =
            target["schema_name"],index = False, if_exists = "append", dtype = dtype_dict)
...
def create(conn, dataframe,conn_details:str,target)-> bool:
    ...
    try:
        if db_table_exists(conn, target["schema_name"],target["object_name"].lower()) is False:
...
def append(conn: dict, dataframe, conn_details,target) -> bool:
    ...
    try:
        if db_table_exists(conn, target["schema_name"], target["object_name"].lower()) is True:
    ...
    try:
        if db_table_exists(conn, target["schema_name"], target["object_name"].lower()) is True:
            if counter == 1:
                ...
                task_logger.info("postgresql truncating table finished, started inserting "
                "data into table :%s", target["object_name"].lower())
                insert_data(conn_details,dataframe,conn,target)
            else:
                insert_data(conn_details,dataframe,conn,target)
        else:
...
def write(json_data,datafram,counter,config_file_path,task_id,run_id,paths_data,
    ...
    """ function for ingesting data to postgresql based on the operation in json"""
    try:
        ...
        task_logger.info("ingest data to postgresql db initiated")
